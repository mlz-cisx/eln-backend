...
def get_stat(db: Session, user):
    ...
    active_users = db.query(models.ActiveUserCount).first().count if db.query(models.ActiveUserCount).first() else 0
    ...
    return StatResponse(total_users=total_users, active_users=active_users, total_labbook=total_labbook,
                        total_notes=total_notes, total_files=total_files, total_pics=total_pics,
                        image_folder_size=image_folder_size, files_folder_size=files_folder_size)
